...
class AES_128
{
    ...
    void addRoundKey(const vector<uint8_t> expandedKey, const int round, vector<uint8_t> &word);
...
public:
    vector<uint8_t> encrypt(const vector<uint8_t> &plainText, const vector<uint8_t> &key);
    vector<uint8_t> decrypt(const vector<uint8_t> &cipherText, const vector<uint8_t> &key);
};